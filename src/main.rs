...
fn memory_usage() {
    if let Some(usage) = memory_stats() {
        info!("Current physical memory usage: {} Mib", usage.physical_mem / (1024*2024));
        info!("Current virtual memory usage: {} Mib", usage.virtual_mem / (1024*2024));
    } else {
    ...
    match &cli.command {
        SubCommands::Completion(completion) => {
            ...
            match completion.shell {
                ...
                Shells::Elvish => generate(Elvish, &mut Cli::command(), "mqc", &mut io::stdout()),
